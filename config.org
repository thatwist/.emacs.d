...
(use-package dired-subtree :ensure :demand :after dired :custom (dired-subtree-line-prefix "    ")) ;;"  └───"
  ...
  (org-agenda-finalize
   . (lambda ()
       (save-excursion ;; doesn't work?
         ...
         ;; ;; (let (window (get-buffer-window (current-buffer)))
         ;; ;;   (window-resize window (- 180 (window-size window t)) t))

         (split-window-below)
         (add-to-list 'golden-ratio-exclude-buffer-names "<current-motivation>")
         (other-window 1)
         (bookmark-jump "current-motivation")
         (when (eq nil (get-buffer "<current-motivation>"))
           (make-indirect-buffer (buffer-name (current-buffer)) "<current-motivation>"))
         (view-buffer "<current-motivation>")
         (org-narrow-to-subtree)
         ;; (outline-mode)
         (visual-line-mode -1)
         (let (window (get-buffer-window (current-buffer)))
...
(use-package org-inline-image-fix 
  ...
  :quelpa (org-http-inline-image
           ...
           :files ("org-http-inline-image.el"))
  :config
