...
** display profile
#+begin_src emacs-lisp
  (defun load-display-profile ()
    "Load the current display profile settings."
    (interactive)
    (let ((profile-file (expand-file-name "lisp/display-profile.el"
  user-emacs-directory)))
      (if (file-exists-p profile-file)
          (progn
            (load-file profile-file)
            ;; (message "Display profile loaded from %s" profile-file)
            )
        (message "Display profile file not found: %s" profile-file))))
#+end_src
** fonts
#+name: fonts
#+begin_src emacs-lisp
  ...
  ;; todo: org this and lisp/display-profile.el
  ;; (set-face-attribute 'default nil :font "0xProto" :height 98)
  ;; (set-face-attribute 'default nil :font "Source Code Pro Medium" :height 98)
  ;; (set-face-attribute 'default nil :font "IBM Plex Mono Text" :height 98)
  (set-face-attribute 'default nil :font "Input Mono Narrow" :height 100)
  ;; WIP - have no idea how emacs handles font aliasing/hinting
  ;; (set-frame-font "InputMonoNarrow-10:antialias=none:hinting=true:hintstyle=hintfull:autohint=false" t t)
  ;; (set-frame-font "InputMonoNarrow-10:antialias=none:hinting=true:hintstyle=hintfull:autohint=true" t t)
  ;; (set-frame-font "InputMonoNarrow-10:antialias=none:hinting=true" t t)

  ;; setting explicit fallback to icon font, otherwise IBMPlexSansTC is picked up (should probably remove that font)
  ;; can put (font-spec :size 50 :name "Miriam Mono CLM:bold") instead of "Miriam Mono CLM:bold"
  (set-fontset-font "fontset-default" '(#xE000 . #xF8FF) "Symbols Nerd Font")
  ;; default fallback, probably fixes some more on top of the above fallback
  (set-fontset-font t nil "Symbols Nerd Font" nil 'append)
  (custom-theme-set-faces
   'user
   ;; '(variable-pitch ((t (:family "Arial" :height 115))))
   ;; '(variable-pitch ((t (:family "IBM Plex Sans" :height 115))))
   ;; '(variable-pitch ((t (:family "Verdana" :height 105))))
   ;; '(variable-pitch ((t (:family "DejaVu Sans" :height 105))))
   '(variable-pitch ((t (:family "Input Sans Narrow" :height 100))))
   ;; '(variable-pitch ((t (:family "Input Sans Narrow" :height 100))))
   ;; '(variable-pitch ((t (:family "Input Serif" :height 102))))
   ;; '(variable-pitch ((t (:family "Input Serif Narrow" :height 102))))
   ;; '(variable-pitch ((t (:family "IBM Plex Serif" :height 102))))
   ;; '(variable-pitch ((t (:family "IBM Plex Sans" :height 102))))
   ;; '(variable-pitch ((t (:family "Georgia" :height 110))))
   '(fixed-pitch ((t ( :family "0xProto" :height 98))))
   ;; '(fixed-pitch ((t ( :family "Input Mono" :height 98))))
   )

  ;; this one basically overrides above but I keep above for easy experiments with fonts
...
;; this messes up symbols actually, e.g.  0xEACE is onion while it has to be database symbol, and it is 8years old font btw
...
(custom-set-faces '(fringe ((t (:weight bold :height 1.2 :width expanded)))))
;; (custom-set-faces '(default ((t (:foreground "#e7e3c9")))))
(custom-set-faces '(default ((t (:background "#ffffff" :foreground "#020507")))))
(custom-set-faces '(highlight ((t (:background "#efeaf9"))))) ;; used by hl-line-mode ;; #f8f7ef
...
(custom-set-faces '(dired-subtree-depth-1-face ((t (:background "#f5f5f5")))))
(custom-set-faces '(dired-subtree-depth-2-face ((t (:background "#f0f0f0")))))
(custom-set-faces '(dired-subtree-depth-3-face ((t (:background "#eaeaea")))))
(custom-set-faces '(dired-subtree-depth-4-face ((t (:background "#e5e5e5")))))
(custom-set-faces '(dired-subtree-depth-5-face ((t (:background "#e0e0e0")))))
(custom-set-faces '(dired-subtree-depth-6-face ((t (:background "#dadada")))))
(custom-set-faces '(dired-subtree-depth-7-face ((t (:background "#d5d5d5")))))
(custom-set-faces '(dired-subtree-depth-8-face ((t (:background "#d0d0d0")))))
(custom-set-faces '(dired-subtree-depth-9-face ((t (:background "#cbcbcb")))))
(custom-set-faces '(dired-subtree-depth-10-face ((t (:background "#c8c8c8")))))
...
(custom-set-faces '(font-lock-constant-face ((t :family "Source Code Pro" :background "#e8eaf8"))))

(custom-set-faces '(font-lock-builtin-face ((t (:box t)))))
(custom-set-faces '(font-lock-number-face ((t (:foreground "#50cbab")))))
(custom-set-faces '(font-lock-doc-face ((t (:slant italic :family "Source Code Pro" :foreground "#aaa")))))
(custom-set-faces '(font-lock-comment-face
                    ((t (:slant italic ;;:weight light
                         :height 0.9 :foreground "#a2a2a2" :family "Source Code Pro")))))

;; otherwise it seems like it is getting overriden somehow
(add-hook 'after-init-hook
          #'(lambda()
              (custom-set-faces '(font-lock-variable-name-face
                                  ((t (:background "#daffef")))))
              ))

(custom-set-faces '(font-lock-function-name-face ((t (:foreground "#64f" :background "#faf7dc")))))
(custom-set-faces '(avy-lead-face ((t (:foreground "#fff" :background "#f5bbd0")))))
(custom-set-faces '(avy-lead-face-0 ((t (:foreground "#fff" :background "#afc7f9")))))
(custom-set-faces '(diredfl-dir-heading ((t (:foreground "#0a0b3a" :background "#ffd7e0")))))
(custom-set-faces '(magit-diff-base ((t (:foreground "#9a9a11" :background "#fefeeb")))))
(custom-set-faces '(magit-diff-base-highlight ((t (:foreground "#8a8a01" :background "#f5f5df")))))
(custom-set-faces '(magit-diff-added ((t (:foreground "#229922" :background "#ddffdd")))))
(custom-set-faces '(magit-diff-added-highlight ((t (:foreground "#1a8a1a" :background "#c3e3c3")))))
(custom-set-faces '(magit-diff-removed ((t (:foreground "#881111" :background "#feecec")))))
(custom-set-faces '(magit-diff-removed-highlight ((t (:foreground "#881111" :background "#f0cdcc")))))
(custom-set-faces '(magit-diff-hunk-heading-highlight ((t (:foreground "#555555" :background "#cecece"
                                                                       ;;:weight semi-bold
                                                                       )))))
(custom-set-faces '(org-done ((t (:box t)))))
;;(custom-set-faces '(org-headline-done ((t (:family "Source Code Pro")))))
#+end_src
...
(use-package evil
  ...
  (with-eval-after-load "evil-maps"
    ...
    ;; free up standard code nav binds
...
** layout
#+begin_src emacs-lisp
(defun insert-canary-helper ()
  "simple cheat sheet to learn my custom canary layout"
  (interactive)
  (insert "
`lypk zfou\\
crstg x eai
...
(use-package files
  ...
  ;; lets keep it light and enable explicitly on demand
...
(use-package dired-subtree :ensure :demand :after dired
  ...
  ;; these are set in the 'other faces' section
  ;; (custom-set-faces
...
** bookmarks
#+begin_src emacs-lisp :tangle no
(customize-set-variable 'bookmark-default-file "~/documents/org/bookmarks.eld")
...
tabs visible in the window currently, without switching to buried/other tabs."
  ...
  (defun my/tab-line-set-faces ()
    ...
    (setq tab-line-tab-face-functions '(my/tab-line-colorize tab-line-tab-face-modified tab-line-tab-face-special))
  )
  ...
  ((tab-line-new-button-show nil)
   ...
   ;;tab-line-tab-face-group tab-line-tab-face-inactive-alternating)
  )
#+end_src
...
(use-package magit
  ...
  (magit-section-mode-map "M-1" nil "M-2" nil "M-3" nil "M-4" nil)
...
** mmm
#+begin_src emacs-lisp
(use-package mmm-mode :ensure
  :config
  (setq mmm-global-mode 'maybe)
  (mmm-add-classes
   '((python-sql
      :submode sql-mode
      :front "\"\"\"[:space:]]*\\(SELECT\\|INSERT\\|UPDATE\\|DELETE\\|WITH\\)"
      :back "\"\"\"")))
...
(use-package graphql-ts-mode :ensure)

(use-package devicetree-ts-mode :ensure
  :demand
...
(use-package rasi-mode :ensure)

(use-package ejson-mode :ensure)

(use-package ess :ensure)

(use-package tree-sitter-ess-r :ensure)

(use-package polymode :ensure)

(use-package poly-R :ensure)

;; experimenting
(use-package poly-erb :ensure
  :config
  (defvar poly-yaml-root-polymode
    (pm-polymode :name "yaml-root" :hostmode 'poly-yaml-hostmode) "yaml root polymode.")
  (define-polymode poly-yaml+erb-mode poly-yaml-root-polymode :innermodes '(poly-erb-innermode))
  )
...
(use-package cargo-mode :ensure
  :demand :after rust-mode
...
(use-package logview :ensure :mode "\\.log\\(?:\\.[0-9]+\\)?\\'")
#+end_src
...
(defun piper-read-region ()
  ...
  (shell-command-on-region
   ...
   (if is-android
       ;; given that termux-api is installed
   "termux-tts-speak -r 0.9"
   ;; else linux
   (concat "piper-tts -q --model ~/documents/en_US-lessac-medium.onnx --sentence_silence 0.8 --output-raw | "
...
(use-package claude-code :ensure :requires vterm)
#+end_src
...
#+begin_src emacs-lisp
(when is-android
  (defun android-browse-url(url &optional share-url-instead)
    "browse url on android termux"
    (shell-command (concat "termux-open-url " url))))
