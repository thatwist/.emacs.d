...
(if (eq system-type 'windows-nt)
    (set-face-attribute 'default nil :font "Fira Mono" :height 110) ;; defaults to 139
  (set-face-attribute 'default nil :font "Fira Mono" :height 110) ;; defaults to 139
...
(use-package tab-bar
            ...
            (let ((buffer-assignment-in-progress t))
              (let (window-configuration-change-hook)
                (let ((tab-name (cdr (assq 'name (tab-bar--current-tab))))
                  ...
                  ;; (when (string= tab-name project-name) (message "debug: buffer already belongs to %s" tab-name))
                  (when (not (string= tab-name project-name))
                    (quit-window) ;; bury
                    (when (not (tab-bar--tab-index-by-name project-name))
                      (let ((tab-bar-new-tab-choice buffer-name))
                        (message (format "creating new tab %s" project-name))
                        (tab-bar-new-tab)
  ...
  ((tab-bar-tab-hints nil) ; t
   (tab-bar-tab-name-function #'(lambda () "-")) ;; experiment
   (tab-bar-show t)
  ...
  (defun set-last-tab-window-quit-restore ()
    ...
    (let* ((tab-line-tabs (tab-line-tabs-window-buffers))
      ...
      (when (= 1 (length tab-line-tabs)) ;; when it's last tab - hide window
        (if (= 1 (length windows))
            (tab-bar-close-tab) ;; when it's last window in the tab-bar-tab - close tab-bar-tab
...
(pretty-hydra-define toggle-hydra
  ...
  ("Emacs"
   (("D" desktop-save-mode "desktop mode" :toggle t)
    ("S" auto-sudoedit-mode "auto-sudoedit-mode" :toggle t)
    ("de" toggle-debug-on-error "debug on error" :toggle debug-on-error)
    ("dq" toggle-debug-on-quit "debug on C-g" :toggle debug-on-quit))
   "Editing"
   (("c" global-company-mode "company" :toggle t)
    ("y" yas-global-mode :toggle t)
   ...
   (("i" ivy-rich-mode "ivy-rich" :toggle t)
    ...
    ("w" global-whitespace-mode "g-whitespace" :toggle t)
    ("r" rainbow-mode "rainbow" :toggle t)
    ("B" page-break-lines-mode "page brks" :toggle t)
    ("L" global-hl-line-mode "g-hl-line" :toggle t)
...
(use-package magit
  ...
  :after evil general
  :general ('normal 'magit-mode-map "?" 'magit-dispatch)
...
(use-package company
  ...
  (define-key company-mode-map [remap indent-for-tab-command] 'company-complete-common-or-cycle)
  ;; tab-and-go
  (company-tng-mode)
...
(use-package lsp-ui
  ...
  ;; :custom-face (lsp-ui-sideline-symbol-info ((t :height 1.55)))
