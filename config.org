...
(use-package emacs
  ...
  :hook
  (after-init . (lambda () ;; these stuff gets overwritten if run during init
                  (tooltip-mode -1)
...
(if (eq system-type 'windows-nt)
  ...
  (set-face-attribute 'default nil :font "Input Mono Narrow" :height 75)
...
(use-package dired-subtree
  ...
  :config
  (evil-collection-define-key '(normal motion) 'dired-mode-map (kbd "M-k") 'kill-this-buffer)
  (evil-collection-define-key '(normal motion) 'dired-mode-map (kbd "M-j") 'bury-buffer))
...
(use-package evil-anzu
  :ensure t :after (evil anzu))

;; evil-anzu actually requires this
(use-package anzu
  :ensure t :init (global-anzu-mode +1))
#+end_src
...
(use-package tab-line
  ...
  (defface tab-line-tab-inactive-dired '((t :inherit tab-line-tab-inactive :foreground "coral")) "face for dired tab-line tabs")
  (defun my/tab-line-tab-face-dired (tab _tabs face buffer-p _selected-p)
    (when (and buffer-p (buffer-file-name tab) (eq (buffer-local-value 'major-mode tab) 'dired-mode))
  ...
  ((tab-line-new-button-show nil)
   ...
   (tab-line-tab-face-functions '(my/tab-line-tab-face-dired tab-line-tab-face-modified tab-line-tab-face-special))) ;;tab-line-tab-face-group tab-line-tab-face-inactive-alternating 
  )
...
(pretty-hydra-define utils-hydra
  ...
  ("Ivy"
   (("v" ivy-push-view "push view")
    ("y" ivy-switch-view "switch view")
    ("V" ivy-pop-view "pop view"))
   "Emacs"
   (("Ps" profiler-start "profile")
    ("Pc" #'(lambda () (interactive) (profiler-start 'cpu)) "profile cpu")
    ("Pm" #'(lambda () (interactive) (profiler-start 'mem)) "profile mem")
    ("Pq" profiler-stop "profile stop")
    ("L" list-packages))
   "User"
   (("ue" elfeed)
    ("um" mu4e)
    ("uM" mu4e-fast-update-mail-and-index)
    ("uc" calculator)
    ("tt" google-translate-smooth-translate)
    ("pp" pass)
    ("pn" password-store-insert)
...
(use-package org
  ...
  :bind (:map org-src-mode-map ("s-'" . org-edit-src-exit))
...
(setq org-todo-keywords
      '(
        ...
        (type "WEAKNESS(n)" "TECHNIQUE(q)" "SKILL(k)")
...
(use-package org-agenda
  ...
  (org-agenda-files
   '(
     ...
     "~/Dropbox/org/dk.gpg" "~/Dropbox/org/content.org" "~/Dropbox/org/goals.org"
     "~/Dropbox/org/consume.org" "~/Dropbox/org/talks.org"
     "~/Dropbox/org/gcal/family.org" "~/Dropbox/org/gcal/trading.org" "~/Dropbox/org/gcal/sport.org"
     "~/Dropbox/org/gcal/personal.org"
        ...
        `(("c" . "Custom Agendas")
          ...
          ("a" "Action"
           (,(agenda-tags-todo-with-goals "⚡ Doing:" " 🔨" "+TODO={IN-PROGRESS\\|DELEGATED}")
            ,(agenda-tags-todo-with-goals "⚡ Next:" " ↷" "-project+PRIORITY={A\\|B}+TODO={ON-HOLD\\|TODO\\|WAITING}")
          ...
          ("A" "Action #A"
           (,(agenda-tags-todo-with-goals "⚡ Doing[A]:" " 🔨" "+TODO={IN-PROGRESS\\|WAITING\\|DELEGATED}+PRIORITY={A}")
            ,(agenda-tags-todo-with-goals "⚡ Next[A]:" " ↷" "-project+PRIORITY={A}+TODO={ON-HOLD\\|TODO\\|WAITING}")
          ...
          ("r" "All Review"
           ((tags "+blocking/!" ((org-agenda-overriding-header "Blocking others")))
            ...
            (tags-todo "-project+PRIORITY={A\\|B}-TODO=\"IN-PROGRESS\"/!-GOAL-DRAFT-TOREAD-IDEA" ((org-agenda-overriding-header "Most important to do")))
            ...
            (tags-todo "-project-book-PRIORITY={A\\|B}-TODO=\"IN-PROGRESS\"/!-WAITING-GOAL-VISION-MODE-FOCUS-SOMEDAY-MAYBE-DRAFT-IDEA-TOREAD-READING" ((org-agenda-overriding-header "Other to do")))
...
(use-package company
  ...
  ;; older logic I used instead of above
...
(use-package company-box
  ...
  ;; todo - this works bad with tab-bar-mode
