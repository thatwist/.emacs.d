...
#+begin_src emacs-lisp
  (use-package minimap
    ...
    (with-eval-after-load 'pretty-hydra
      (pretty-hydra-define+ toggle-hydra nil
        ("Text" (("M" minimap-mode "minimap-mode" :toggle t))))))
#+end_src
...
(setq org-capture-templates
      '(
        ("c" "calfw2org" entry (file "~/Dropbox/org/inbox.org")  "* %? %(cfw:org-capture-day)")
...
(setq org-todo-keywords
      '(
        (sequence "TODO(t)" "DRAFT(l)" "IN-PROGRESS(i)" "TRACK(T)" "PROCESS(P)" "WAITING(w!)" "DELEGATED(e@/!)" "ON-HOLD(h@/!)" "|")
        (sequence "IDEA(i)" "MAYBE(m)" "SOMEDAY(s)" "PROJECT(p)" "|")
        (sequence "VISION(v)" "GOAL(g)" "FOCUS(f)" "MODE(o)" "|")
        (type "WEAKNESS(n)" "TECHNIQUE(q)" "HABIT(j)" "SKILL(k)" "CHEATSHEET(a)" "RESOLUTION(r)" "test(6)")
        (type "BOOK(z)" "|")
...
(with-eval-after-load 'org-agenda
  (setq org-agenda-custom-commands
        `(("c" . "Custom Agendas")
          ...
          ("ci" "All In Progress" ((todo "IN-PROGRESS|PROCESS|TRACK")) ((org-agenda-max-entries 25)) nil)
          ...
          ("a" "Action"
           (,(agenda-tags-todo-with-goals "⚡ Goals:" " 🟍" "+current+goal")
            ...
            ,(agenda-tags-todo-with-goals "⚡ Process:" " 🔨" "+TODO={PROCESS\\|TRACK}")
           ...
           ("agenda.html"))
          ("A" "Action #A"
           (,(agenda-tags-todo-with-goals "⚡ Doing[A]:" " 🔨" "+TODO={IN-PROGRESS\\|PROCESS\\|TRACK\\|WAITING\\|DELEGATED}+PRIORITY={A}")
...
#+begin_src emacs-lisp
(defun rclone-dropbox-bisync ()
  "Dropbox sync helper using rclone."
  (interactive)
  (let ((buf "rclone-dropbox-bisync"))
    (switch-to-buffer buf)
    (erase-buffer)
    (call-process "rclone" nil buf t
                  "bisync" "--verbose" "dropbox:" (concat (getenv "HOME") "/Dropbox"))
    (run-with-idle-timer 3 nil `(lambda () (kill-buffer ,buf)))))
(add-hook 'kill-emacs-hook #'rclone-dropbox-bisync)
(add-hook 'after-init-hook #'rclone-dropbox-bisync)
(with-eval-after-load 'pretty-hydra
  (pretty-hydra-define+ utils-hydra ()
    ("User" (("ud" rclone-dropbox-bisync "dropbox sync")))))
#+end_src
