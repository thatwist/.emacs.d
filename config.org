...
(use-package edebug
  ...
  :bind
  (:map edebug-mode-map ("?" . hydra-edebug/body)))
#+end_src
...
(use-package helpful
  ; todo - remove this init bindings?
  :init
  ...
  :config (evil-define-key 'motion global-map (kbd "<leader>h") hydra-help/body)
  )
...
(use-package evil-collection
  ...
  :pretty-hydra (help-hydra ("Evil" (("E" evil-collection-describe-bindings "evil-collection"))))
  )
...
(with-eval-after-load 'evil 
  ...
  (evil-define-key 'motion global-map 
     ...
     (kbd "<leader>u") 'hydra-util/body
     (kbd "<leader>m") 'major-mode-hydra
     (kbd "<leader>h") 'hydra-help/body
     (kbd "<leader>M") 'mu4e
...
;; lets keep it until tab-bar-history is fixed
(winner-mode 1)
...
(global-set-key (kbd "C-S-k") 'bury-buffer) ; this is to emulate tab-line-close-tab, in fact it is the same effect
...
#+begin_src emacs-lisp
(pretty-hydra-define hydra-util
  ...
  (let ((file (buffer-file-name)))
    (async-start
     ...
     `(lambda (result)
        ...
        ;; I don't like this
        (when (string-match-p "config.el" (car result))
...
(use-package org-roam
  :preface (setq org-roam-directory "path/to/org/files") ;; I want to silence bytecompiler
  :ensure t
...
(use-package ediff
  :ensure nil
  :custom-face (ediff-current-diff-a ((t (:background "saddle brown" :extend t))))
...
#+begin_src emacs-lisp
(customize-set-variable 'custom-file 'null-device "not storing")
