...
(use-package gruvbox-theme
  ...
  (defun my/toggle-light-dark ()
    ...
    (if (eq (car custom-enabled-themes) 'gruvbox-light-soft)
        (progn (counsel-load-theme-action "gruvbox-dark-hard") (setq my/toggle-dark t))
      (progn (counsel-load-theme-action "gruvbox-light-soft") (setq my/toggle-dark nil))))
  :config
...
(use-package good-scroll
  ...
  (define-key evil-motion-state-map (kbd "s-j") 'good-scroll-up-full-screen)
  (define-key evil-motion-state-map (kbd "s-k") 'good-scroll-down-full-screen)
...
(use-package dired-subtree
  :ensure t) ;; evil collection takes care of bindings
...
(use-package dired
  ...
  (evil-collection-setup-hook . (lambda (mode map)
                                  (when (eq mode 'dired)
...
(use-package tab-line
  ...
  ("M-n" . next-buffer)
  ("M-p" . previous-buffer)
  ("M-o" . bury-buffer) ; same as closing tab in tab-line
  ("M-C-o" . kill-current-buffer) ;; don't have better binding left
  ("C-S-k" . kill-current-buffer)
  ;; todo - this fails here cause term is not loaded yet
  ;;(:map term-raw-map ("M-n" . next-buffer) ("M-p" . previous-buffer))
  ;;(:map term-mode-map ("M-n" . next-buffer) ("M-p" . previous-buffer))
  ;;(:map eshell-hist-mode-map ("M-n" . next-buffer) ("M-p" . previous-buffer))
...
(defun agenda-in-progress (&optional priority)
  `(tags-todo ,(if priority
                   (concat "+TODO=\"IN-PROGRESS\"+PRIORITY=\"" priority "\"|+TODO=\"WAITING\"+PRIORITY=\"" priority "\"")
                 (concat "+TODO=\"IN-PROGRESS\"|+TODO=\"WAITING\""))
...
(use-package ediff
  ...
  (defun my/ediff-merge (a b c)
    ...
    (my/ediff-prepare)
    (ediff-merge-files-with-ancestor a b c nil c))
  :commands (ediff)
  :after evil-collection
  :init
